...
banner_html = """
    <header>
        <h1>Customize Cover Letter<br> <span>[ Step Four ]</span></h1>
...
def parse_job_desc(resume_text, job_description):
    with st.spinner("Generating Custom Cover Letter..."):
        ...
        cover_letter(resume_text, summarized_description)

def cover_letter(resume_text, job_description):
    ...
    try:
        ...
        response = client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": "You are a career and resume expert. \
                                Create a cover letter for the applicant, highlighting their skills and experience in relation to the job.",
                },
    ...
    stx.scrollableTextbox(final_letter, height=800)

if 'resume_text' not in st.session_state or 'job_description' not in st.session_state:
    st.error("Please Complete Step 1 and Step 2 to get a custom resume.")
else:
